...
import PIL.Image
import dnnlib
from src.DiffMemorize.optim import OptimEDM
...
@click.option('--seeds', help='Random seeds (e.g. 1,2,5-10)', metavar='LIST', type=parse_int_list, default='0-63',
              show_default=True)
...
def main(network_pkl, outdir, subdirs, seeds, class_idx, max_batch_size, device=torch.device('cuda'), **sampler_kwargs):
    ...
    dist.print0(f'Loading network from optim edm ...')
    net = OptimEDM(
        data_path=network_pkl,
        device=device,
    )
